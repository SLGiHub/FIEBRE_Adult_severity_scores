...
knit: (
  function(inputFile, encoding) { 
    ...
    pSubTitle <- paste0('analysis_reports/adult_severity_scores_', as.character(Sys.Date()))
...
data_file <- c("/Users/sham/Filr/My Files/sync/My Files/fiebre/data/analysis_scripts/adult_severity_scores/data/fiebre_adult_child_sites_combined_2023-06-16.csv")
...
severity_data <- fiebre_data %>%
  select(consent_participate, patient_id_calculate, date_day0_visit, country, sex, px_group, modGCS, qsofa, uva, uva_age, crb_65, mews, cf_9_px_status_final, cf_9_date_death, cf_4_discharge_outcome, cf_4_other_specify, discharge_outcome, cf_5_inpatient_outcome, cf_5_inpatient_outcome_other, cf_5_discharge_date, hiv_comb, age, tx_any_antimicro, hiv, hiv_test_final, hiv_comb, cf_hiv_hiv_test_done, cf_hiv_no_hiv_other, temp, respiratory_rate, respiratory_rate_explain, bp_systolic, bp_diastolic, heart_rate, o2_sats, px_orient, px_eyes, px_eyes_na, px_eyes_no, qsofa_hiv, mews_hiv) %>%
...
unique(severity_data$outcome_discharge)
...
str(severity_data$px_group)
unique(severity_data$px_group)
...
x <- severity_data %>%
  select(country, uva, hiv_comb, hiv, cf_hiv_hiv_test_done, cf_hiv_no_hiv_other, hiv_test_final, temp, heart_rate, respiratory_rate, bp_systolic, o2_sats, modGCS) %>%
  filter(as.character(hiv_comb) == "Unknown")
...
tbl_incomp_data <- tbl_incomp_data %>%  
  tbl_summary( 
    by = country,
    type = list(c("age", "temp", "time_to_discharge", "respiratory_rate", "bp_systolic", "heart_rate", "mews" ,"o2_sats", "modGCS", "uva") ~ 'continuous2'),
    statistic = list(c("age", "temp", "respiratory_rate", "bp_systolic", "heart_rate", "mews" ,"o2_sats", "modGCS", "uva") ~ c("{median} ({p25}, {p75})", "{min}, {max}")
                     ),
    digits = all_continuous() ~ 1,
    missing_text = "(Missing)"
      ) %>%
...
```{r, tab.cap = "UVA Sensitivity and specificity values", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T }
...
```{r, tab.cap = "MEWS Sensitivity and specificity values", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T }
...
```{r, tab.cap = "qSOFA Sensitivity and specificity values", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T }
...
```{r, tab.cap = "Sensitivity analysis (a) assuming all missing a day 28 outcome are alive", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T }
...
\newpage
### Figure S2
```{r all_rocs_sensitivity_alive, fig.cap = "Sensitivity analysis (a), assuming all missing are alive, ROCs for S1) UVA, S2) MEWS, S3) qSOFA ", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T, fig.width = 9, fig.height=6}
...
```{r, tab.cap = "Sensitivity analysis (a) assuming all missing a day 28 outcome died", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T }
...
\newpage
### Figure S3
```{r all_rocs_sensitivity_dead, fig.cap = "Sensitivity analysis (b), assuming all missing day 28 outcomes died, ROCs for S4) UVA, S5) MEWS, S6) qSOFA ", include = T, results = 'asis', echo = F, message = F, warning = F, paged.print = F, eval = T, fig.width = 9, fig.height=6}
...
org_cohort <- boxGrob(glue("Total number of adult patients with fever recruited",
                           "n = {pop}",
                           pop = txtInt(total_px),
                           .sep = "\n"),
                      x = .5,
...
included <- boxGrob(glue("Adult patients with complete data analysed",
                         "n = {sample}",
